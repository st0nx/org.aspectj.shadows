...
protected /*AspectJ Extension */ final void optimizedConcatNodeLists() {
...
protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
	...
	if(isDietParse) {
		...
		DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, oldFirstToken, start, end, intervalToSkip[0], intervalToSkip[1], intervalToSkip[2]);
		diagnoseParser.diagnoseParse();
		
		reportSyntaxErrorsForSkippedMethod(types);
		this.scanner.resetTo(start, end);
	} else {
		DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, oldFirstToken, start, end);
		diagnoseParser.diagnoseParse();
	}
}
			...
			if(methods != null) {
				for (int j = 0; j < methods.length; j++) {
					...
					if(methods[j].errorInSignature) {
						DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, TokenNameDIVIDE, method.declarationSourceStart, method.declarationSourceEnd);
						diagnoseParser.diagnoseParse();
					}
				}
			}
				...
				for (int j = 0; j < length; j++) {
					if (fields[j] instanceof Initializer) {
						...
						if(initializer.errorInSignature){
							DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, TokenNameRIGHT_SHIFT, initializer.declarationSourceStart, initializer.declarationSourceEnd);
							diagnoseParser.diagnoseParse();
						}
					}
				}
