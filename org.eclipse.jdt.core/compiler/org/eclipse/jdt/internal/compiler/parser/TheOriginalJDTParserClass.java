 ...
 */
public class TheOriginalJDTParserClass implements BindingIds, ParserBasicInformation, TerminalTokens, CompilerModifiers, OperatorIds, TypeIds {
...
public TheOriginalJDTParserClass(ProblemReporter problemReporter, boolean optimizeStringLiterals) {
	...
	// javadoc support
	this.javadocParser = new JavadocParser((Parser)this); // AspectJ Extension - added cast	
}
/**
 *
 * INTERNAL USE-ONLY
...
public RecoveredElement buildInitialRecoveryState(){
	...
	if (referenceContext instanceof CompilationUnitDeclaration){
		element = new RecoveredUnit(compilationUnit, 0, (Parser)this); // AspectJ Extension - added cast
		...
		if (referenceContext instanceof AbstractMethodDeclaration){
			element = new RecoveredMethod((AbstractMethodDeclaration) referenceContext, null, 0, (Parser)this); // AspectJ Extension - added cast
			...
			if (referenceContext instanceof TypeDeclaration){
				...
				for (int i = 0; i < type.fields.length; i++){
					...
					if (field != null
						...
						element = new RecoveredInitializer(field, null, 1, (Parser)this); // AspectJ Extension - added cast
...
protected void consumeClassDeclaration() {
	...
	boolean hasConstructor = typeDecl.checkConstructors((Parser)this); // AspectJ Extension - added cast
	...
	public void getMethodBodies(CompilationUnitDeclaration unit) {
		...
		if (unit.types != null) {
			for (int i = unit.types.length; --i >= 0;)
				unit.types[i].parseMethod((Parser)this, unit); // AspectJ Extension - added cast
		}
			...
			if (act <= NUM_RULES)
				...
				if (act > ERROR_ACTION) { /* shift-reduce */
					...
					try{
						...
						if (!hasReportedError){
							this.problemReporter().scannerError((Parser)this, e.getMessage()); // AspectJ Extension - added cast
...
protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
	...
	if(isDietParse) {
		...
		DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, oldFirstToken, start, end, intervalToSkip[0], intervalToSkip[1], intervalToSkip[2]); // AspectJ Extension - added cast
		diagnoseParser.diagnoseParse();
		
		reportSyntaxErrorsForSkippedMethod(types);
		scanner.resetTo(start, end);
	} else {
		DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, oldFirstToken, start, end); // AspectJ Extension - added cast
		diagnoseParser.diagnoseParse();
	}
}
			...
			if(methods != null) {
				for (int j = 0; j < methods.length; j++) {
					...
					if(methods[j].errorInSignature) {
						DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, TokenNameDIVIDE, method.declarationSourceStart, method.declarationSourceEnd); // AspectJ Extension - added cast
						diagnoseParser.diagnoseParse();
					}
				}
			}
				...
				for (int j = 0; j < length; j++) {
					if (fields[j] instanceof Initializer) {
						...
						if(initializer.errorInSignature){
							DiagnoseParser diagnoseParser = new DiagnoseParser((Parser)this, TokenNameRIGHT_SHIFT, initializer.declarationSourceStart, initializer.declarationSourceEnd); // AspectJ Extension - added cast
							diagnoseParser.diagnoseParse();
						}
					}
				}
